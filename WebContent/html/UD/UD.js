...
$(document).ready(function(){
	...
	$('input[name=udRadio]:radio').unbind('change').bind('change', function(event) {
		var UDRadio = event.target;
		var UDRadioVal ="";
		if(UDRadio.checked){
			UDRadioVal = UDRadio.value;
//			console.log("UD"+radioValue);
//			document.getElementById("udRadioValue").innerHTML = radioValue;
			switchUdPriceType(UDRadioVal);
...
//		filterList_multi("UdDetailList",[{field: "acnt_date",op:"eq",data: acntDate},{field: "price_type",op: "eq",data: 1}]);
//	jqGrid_UdDetailList("#UdDetailList","#UdDetailList_Pager",setUdPriceTypeArray(UDArray,"1"));	
	break;
	
	case '處置':
		filterList_single("UdDetailList","price_type",2);
//	jqGrid_UdDetailList("#UdDetailList","#UdDetailList_Pager",setUdPriceTypeArray(UDArray,"2"));	
	break;
	
	case '材料':
		filterList_single("UdDetailList","price_type",3);
...
//	 resizePanelBody("jqGridTakeHomeList_BgStyle",(UdGridHeight+100));
	 var tableGrid = $(tableName);
	 	$(tableName).jqGrid({
	    datatype: "local",
	    rowNum: 100,
...
//			{ label: '單位', name: 'unit', width: 70,align:'center'},
	        { label: '非常規', name: 'unuse', width: 68,align:'left'},
	        { label: '常規<br/>用法', name: 'use', width: 70,align:'left'},
...
//			{ label: '單位', name: 'unit', width: 60,align:'center'},
			...
			{ label: '自費', name: 'self', width: 45,align:'center'},
			{ label: '庫別', name: 'stock', width: 55,align:'left'},
...
//			{ add: false, edit: false, del: false, search: true, view: true, refresh: false });
	...
	function getSelectedRow() {
		...
		if(rowKey){
//			var acnt_date = $(tableName).jqGrid('getCell',rowKey,'acnt_date');
//			filterList_single("UdDetailList","acnt_date",acnt_date);
